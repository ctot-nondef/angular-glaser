...
AdlibServices.service('opacsearch', ['$http', '$q', function($http, $q){
  ...
  var getFullList = function(database){console.log('getFullList Query: ', database);
	  if(database) var promise = $http.get(urlList.baseURL+"database="+database+"&search=all&output=JSON&limit=1000");
	  else var promise = $http.get(urlList.baseURL+"search=all&output=JSON&limit=1000");
	  deferObject =  $q.defer();
	  promise.then(
  ...
  var getPointerList = function(database, pointerfile){console.log('getPointerList Query: ', database, pointerfile);
	  if(database && pointerfile) var promise = $http.get(urlList.baseURL+"database="+database+"&search=pointer "+pointerfile+"&output=JSON&limit=100");
  ...
  var getSingleRecord = function(database, reference){console.log('getSingleRecord Query: ', database, reference);
	  if(database && reference) var promise = $http.get(urlList.baseURL+"action=search&database="+database+"&search=priref="+reference+"&output=JSON&limit=100");
	  else console.log('Parameters Missing'); 
	  deferObject =  $q.defer();
	  promise.then(
		function(resp){ deferObject.resolve(resp); /*console.log('$state: ', $state);*/ },
